<template>
  <a-layout id="main" style="height:100%">
    ...
    >
      <div class="logo" />
      <a-menu theme="dark" mode="inline" :defaultSelectedKeys="['1']" :openKeys="openKeys" @openChange="onOpenChange" @click="handleChange">
        <a-menu-item key="主页">
          <a-icon type="user" />
          <span>主页</span>
        </a-menu-item>
        ...
        <a-sub-menu key="sub1" v-if="isAdmin||isTest">
            <span slot="title"><a-icon type="mail"/><span>教务员管理</span></span>
        ...
        <a-sub-menu key="sub2" v-if="isAcademic||isTest">
            <span slot="title"><a-icon type="mail"/><span>教师管理</span></span>
        ...
        <a-sub-menu key="sub3" v-if="isAcademic||isTest">
            <span slot="title"><a-icon type="mail"/><span>学生管理</span></span>
        ...
        <a-sub-menu key="sub4" v-if="isTeacher||isTest">
            <span slot="title"><a-icon type="mail"/><span>申报管理</span></span>
        ...
        <a-sub-menu key="sub5" v-if="!isAdmin||isTest">
            <span slot="title"><a-icon type="mail"/><span>教务公告</span></span>
        ...
        <a-sub-menu key="sub6" v-if="isTeacher||isStudent||isTest">
            <span slot="title"><a-icon type="mail"/><span>课程管理</span></span>
    ...
    <a-layout>
      ...
      <a-layout-content :style="{ margin: '24px 16px', padding: '2px', background: '#fff', minHeight: '280px' }">
...
export default {
  ...
  components:{
      Login,home,setProfile,reviewList,reviewDetail,reviewResulter,
      academicList,addAcademic,teacherList,addTeacher,studentList,addStudent,
      declareList,declareCourse,declareDetail,announceList,createAnnounce,
      myCourse,courseInfo,courseAnnounceList,courseware,
      homeworkList,dataHomework,homeworkReviewer,studentHomework,courseClass,classStudent,homeworkPublisher,
      publishCourseAnn,annConent,AnnouncementEdit,readCourseAnn,
  },
  data:function(){
      return {
          content:"home",
